...
/*edit_style("filter: hue-rotate(0deg) invert(100%)");
 */
...
browser.storage.local.get(domain).then(function (black_list_re_invert){ 
    if(global_turn_off){
        ...
        toggle = !toggle;
		insert_css(modifier, "filter:hue-rotate(180deg)"); //for keep inverted
    } else {
        if(global_hide){
            insert_css(modifier, "filter: opacity(0);"); 
        } else {
            if (isEmpty(black_list_re_invert)) {//if not in black list
                insert_css(modifier, "filter: contrast(95%) brightness(95%) hue-rotate(0deg) invert(100%);"); //for re-inverted
            } else { 
                toggle = !toggle;
                insert_css(modifier, "filter:hue-rotate(180deg)"); //for keep inverted
            }   
...
function switching(evt) {
	...
	if (altPressed && ctrlPressed) {
		...
		case 90: //'z':
            ...
            inversion_type = toggle ? "filter:hue-rotate(180deg)": "filter:hue-rotate(0deg) invert(100%);";
			can_hide = true;
            toggle = !toggle;
		...
		case 65: //'a'
			...
			can_hide = true;
            toggle = true;
			inversion_type = "filter: contrast(95%) brightness(95%) hue-rotate(0deg) invert(100%);";
			replace_css(modifier, inversion_type);
		...
		case 85: //u
			//toggle pdf 
			inversion_type = html_toggle? "": "filter:hue-rotate(0deg) invert(100%);";
