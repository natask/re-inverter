...
chrome.storage.sync.get("global_hide").then(function (key_value) {
	global_hide = key_value["global_hide"];
});
chrome.storage.sync.get("global_turn_off").then(function (key_value) {
	global_turn_off = key_value["global_turn_off"];
});
chrome.storage.sync.get("global_hue_rotate").then(function (key_value) {
...
chrome.storage.sync.get(domain).then(function (domain_settings) {
...
function switching(evt) {
	...
	if (altPressed && ctrlPressed) {
		...
		case 90: //'z':
			...
			inversion_type = toggle ? "" : "filter:hue-rotate(" + global_hue_rotate + "deg) invert(100%);";
		...
		case 85: //u
			//to toggle pdf
			inversion_type = html_toggle ? "" : "filter:hue-rotate(" + global_hue_rotate + "deg) invert(100%);";
			...
			if (can_set_global_hue_rotate) {
				chrome.storage.sync.get("global_hue_rotate").then(function (key_value) {
					chrome.storage.sync.set({
						...
						alert('global hue-rotate is now ' + global_hue_rotate);
					});
				});
			}
			break;
		...
		case 66: //b
			...
			var url = new URL(document.URL);
			var domain = url.hostname;
			chrome.storage.sync.get(domain).then(function (black_list_re_invert) {
				var url = new URL(document.URL);
				var domain = url.hostname;
				if (isEmpty(black_list_re_invert)) { //if not in blacklist, enter blacklist
					...
					chrome.storage.sync.remove(domain).then(function () {
