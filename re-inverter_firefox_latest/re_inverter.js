...
var global_turn_off;
var global_hide;
browser.storage.local.get("global_hide").then(function (key_value) {
	global_hide = key_value["global_hide"];
});
browser.storage.local.get("global_turn_off").then(function (key_value) {
...
*/
var modifier = 'img,video,iframe,canvas,object,svg';//svg added
var url = new URL(document.URL);
var domain = url.hostname;

browser.storage.local.get(domain).then(function (black_list_re_invert){ 
    if(global_turn_off){
        toggle = !toggle;
		insert_css(modifier, ""); //for keep inverted
    } else {
        if(global_hide){
            insert_css(modifier, "filter: opacity(0) !important;"); 
        } else {
            if (isEmpty(black_list_re_invert)) {//if not in black list
                insert_css(modifier, "filter: contrast(95%) brightness(95%) hue-rotate(180deg) invert(100%) !important;"); //for re-inverted
            } else { 
                toggle = !toggle;
                insert_css(modifier, ""); //for keep inverted
            }   
...
function switching(evt) {
	...
	if (altPressed && ctrlPressed) {
		...
		case 90: //'z':
            ...
            modifier = 'img,video,iframe,canvas,object,svg';//svg added
            inversion_type = toggle ? "": "filter:hue-rotate(180deg) invert(100%) !important;";
			can_hide = true;
            toggle = !toggle;
		...
		case 65: //'a'
			...
			can_hide = true;
            toggle = true;
			inversion_type = "filter: contrast(95%) brightness(95%) hue-rotate(180deg) invert(100%) !important;";
			replace_css(modifier, inversion_type);
		...
		case 83: //'s':
			...
			can_hide = true;
            toggle = true;
			inversion_type = "filter:hue-rotate(180deg) invert(100%) !important;";
			replace_css(modifier, inversion_type);
			replace_youtube_css(true);
			break;
		...
		case 68: //'d':       
			//toggle global_hide
            inversion_type = "filter: opacity(0) !important;";              
		...
		case 66: //b
			...
			browser.storage.local.get(domain).then(function (black_list_re_invert) {
				...
				if (isEmpty(black_list_re_invert)){ //if not in blacklist, enter blacklist
					...
					browser.storage.local.remove(domain).then(function () {
		...
		case 85: //u
			//toggle pdf 
			inversion_type = html_toggle? "": "filter:hue-rotate(180deg) invert(100%) !important;";
